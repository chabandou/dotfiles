...
Item {
  ...
  // Track opening animation lifecycle (used to prevent horizontal drift)
  property bool openingAnimationActive: false
  ...
  // Panel-level animation preference (can also be set by content for plugin panels)
  property bool preferBottomAnimation: false
  // Panel-level positioning preference (center across full screen even with vertical bar)
  property bool preferFullScreenCentering: false
  readonly property bool effectivePreferBottomAnimation: preferBottomAnimation || (contentLoader.item && contentLoader.item.preferBottomAnimation === true)
  ...
  function closeImmediately() {
    ...
    root.openingAnimationActive = false;
  ...
  function setPosition() {
    ...
    if (root.useButtonPosition && root.width > 0 && panelWidth > 0) {
      ...
      if (root.panelAnchorHorizontalCenter) {
        if (root.barIsVertical) {
          if (root.preferFullScreenCentering) {
    ...
    if (panelBackground.forceVerticalAnimation && root.openingAnimationActive) {
      panelBackground.deferredTargetX = calculatedX;
      panelBackground.hasDeferredTargetX = true;
    } else {
      panelBackground.targetX = calculatedX;
      panelBackground.hasDeferredTargetX = false;
    }
  ...
  Item {
    ...
    Item {
      ...
      property real deferredTargetX: 0
      property bool hasDeferredTargetX: false
      ...
      // Force vertical bottom animation for bottom-centered panels or panels that explicitly prefer it
      readonly property bool forceVerticalAnimation: root.panelAnchorBottom && (root.panelAnchorHorizontalCenter || root.effectivePreferBottomAnimation)
      ...
      readonly property bool animateFromBottom: {
        ...
        if (root.effectivePreferBottomAnimation && root.panelAnchorBottom) {
          return true;
        }
        if (!panelContent.allowAttach || !isActuallyAttachedToAnyEdge) {
          return false;
        ...
        if (!root.isPanelVisible) {
          ...
          // Panel anchored to top edge - don't animate from bottom
          if (root.panelAnchorTop) {
            return false;
          }
          // Attached to bottom edge (when bar is vertical, panel can still be anchored to bottom)
          // Prefer bottom animation even if also anchored left/right.
          if (panelContent.allowAttach && root.panelAnchorBottom) {
            return true;
          }
          // Panel anchored to left/right edge only - don't animate from bottom
      ...
      readonly property bool shouldAnimateHeight: forceVerticalAnimation || animateFromTop || animateFromBottom
      ...
      Behavior on width {
        enabled: !PanelService.closedImmediately && !panelBackground.forceVerticalAnimation
        NumberAnimation {
          id: widthAnimation
      ...
      Behavior on height {
        ...
        NumberAnimation {
          ...
          onRunningChanged: {
            ...
            // When open height animation completes, apply deferred targetX (if any)
            if (!running && root.isPanelVisible && !root.isClosing && root.openingAnimationActive) {
              root.openingAnimationActive = false;
              if (panelBackground.hasDeferredTargetX) {
                panelBackground.targetX = panelBackground.deferredTargetX;
                panelBackground.hasDeferredTargetX = false;
              }
            }
          }
    ...
    Loader {
      ...
      MouseArea {
        id: contentHoverArea
        anchors.fill: parent
        enabled: root.isPanelOpen && contentLoader.active
        hoverEnabled: true
        acceptedButtons: Qt.NoButton
        propagateComposedEvents: true
        onEntered: PanelService.setHoverPanelHovered(root, true)
        onExited: PanelService.setHoverPanelHovered(root, false)
      }
      ...
      onLoaded: {
        function finalizeOpen() {
          ...
          if (!root.width || !root.height || panelBackground.targetWidth <= 0 || panelBackground.targetHeight <= 0) {
            Qt.callLater(finalizeOpen);
            return;
          }
          ...
          var forceBottom = panelBackground.forceVerticalAnimation;
          root.cachedAnimateFromTop = forceBottom ? false : panelBackground.animateFromTop;
          root.cachedAnimateFromBottom = forceBottom ? true : panelBackground.animateFromBottom;
          root.cachedAnimateFromLeft = forceBottom ? false : panelBackground.animateFromLeft;
          root.cachedAnimateFromRight = forceBottom ? false : panelBackground.animateFromRight;
          root.cachedShouldAnimateWidth = forceBottom ? false : panelBackground.shouldAnimateWidth;
          root.cachedShouldAnimateHeight = forceBottom ? true : panelBackground.shouldAnimateHeight;

          // Make panel visible, now only the intended dimension will animate
          root.openingAnimationActive = true;
          root.isPanelVisible = true;
        ...
        // Wait for contentPreferredWidth/Height and root dimensions to be available
        Qt.callLater(finalizeOpen);
      }
    }
  }
