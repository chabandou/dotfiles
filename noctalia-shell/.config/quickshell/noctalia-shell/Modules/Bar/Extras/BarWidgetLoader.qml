...
Item {
  ...
  readonly property string hoverPanelName: PanelService.panelNameForBarWidget(root.widgetId)
  ...
  HoverHandler {
    ...
    enabled: root.hoverPanelName !== "" && !!loader.item
    target: loader.item

    // Track hover state without taking pointer grabs so wheel/click stay on widget.
    onHoveredChanged: {
      if (!loader.item || root.hoverPanelName === "")
        return;
      PanelService.updateHoverWidgetState(root.widgetId, root.widgetScreen, loader.item, hovered);
    }
  }
